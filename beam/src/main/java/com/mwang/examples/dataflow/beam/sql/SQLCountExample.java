package com.mwang.examples.dataflow.beam.sql;
...
public class SQLCountExample {
    ...
    public static void main(String[] args) {
        ...
        PCollection<String> fileInput = pipeline.apply(TextIO.read().from(path + "\\user_order.csv"));
        ...
        PCollection<Row> sqlInput = rowInput
                .apply(SqlTransform.query("select userId,Count(userId) from PCOLLECTION group by userId "));

        // Step 4 : Convert PCollection<Row> to PCollection<String>
        PCollection<String> pOutput = sqlInput.apply(ParDo.of(new RowToString()));
        pOutput.apply(TextIO.write().to(path + "\\sql_count_output.csv").withNumShards(1).withSuffix(".csv"));
        pipeline.run();
    }
