...
public class SQLJoinExample {
    ...
    public static void main(String[] args) {
        ...
        PCollection<String> order = pipeline.apply(TextIO.read().from(path + "\\user_order.csv"));
        PCollection<String> user = pipeline.apply(TextIO.read().from(path + "\\p_user.csv"));
        ...
        pOutput.apply(TextIO.write().to(path + "\\sql_join_output.csv").withNumShards(1).withSuffix(".csv"));
        pipeline.run();
    }
