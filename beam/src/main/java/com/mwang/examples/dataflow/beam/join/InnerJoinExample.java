package com.mwang.examples.dataflow.beam.join;
...
public class InnerJoinExample {
    ...
    public static void main(String[] args) {
        String path = "C:\\mwang/works/gcp/examples/beam/data";

        Pipeline p = Pipeline.create();

        // Step 1 : Convert String to KV object.

        PCollection<KV<String, String>> pOrderCollection = p.apply(TextIO.read().from(path + "/user_order.csv"))
                .apply(ParDo.of(new OrderParsing()));

        PCollection<KV<String, String>> pUserCollection = p.apply(TextIO.read().from(path + "/p_user.csv"))
        ...
        PCollection<KV<String, CoGbkResult>> result = KeyedPCollectionTuple.of(orderTuple, pOrderCollection)
                .and(userTuple, pUserCollection).apply(CoGroupByKey.<String>create());
        ...
        output.apply(TextIO.write().to(path + "/cogroup_by_key.csv").withNumShards(1).withSuffix(".csv"));
        p.run();
